...
export class ColorSchemeManager {
  ...
  constructor(
    ...
    this.detectDark.trigger((isDark: boolean) => {
      if ((isDark && (this.currentTheme === 'dark'))
  ...
  getCurrent(): string {
    let theme = this.storage.getItem(this.storageKey);
    this.currentTheme = theme ||
      (this.detectDark.isDark() && 'dark' || this.defaultTheme);
  ...
  checkDarkHTML(): boolean {
    let mark = document.documentElement.getAttribute(this.domMark);
    return mark === 'dark';
  }
  symCheck(theme: string): boolean {
    let isDark = this.detectDark.isDark();
...
const colorSchemeManager = new ColorSchemeManager('light', {
  'light': 'dark',
  'dark': 'light',
